 ...
 * 
 * 
 * ++ Added SmoothStep and SmootherStep @glbs
 */
 ...
 */
    ...
    */
	...
	public static Function Ease(EaseType type) {
		...
		case EaseType.SmoothStep: f = Interpolate.SmoothStep; break;
		case EaseType.SmootherStep: f = Interpolate.SmootherStep; break;
		case EaseType.EaseOutInExpo: f = Interpolate.EaseOutInExpo; break;
		}
		return f;
	}
	...
	//SmoothStep
	static float SmoothStep(float start, float distance, float elapsedTime, float duration)
	{
		// clamp elapsedTime so that it cannot be greater than duration
		elapsedTime = (elapsedTime > duration) ? 1.0f : elapsedTime / duration;
		return distance * (elapsedTime * elapsedTime * (3 - 2 * elapsedTime)) + start;
	}


	//SmoothStep
	static float SmootherStep(float start, float distance, float elapsedTime, float duration)
	{
		// clamp elapsedTime so that it cannot be greater than duration
		elapsedTime = (elapsedTime > duration) ? 1.0f : elapsedTime / duration;
		return distance * (elapsedTime * elapsedTime * elapsedTime * ( elapsedTime * (elapsedTime * 6 - 15) + 10)) + start;
	}


	//EASE OUT IN EXPO TEST
	static float EaseOutInExpo(float start, float distance, float elapsedTime, float duration)
	{
		// clamp elapsedTime so that it cannot be greater than duration
		elapsedTime = (elapsedTime > duration) ? 2.0f : elapsedTime / (duration / 2);
		if (elapsedTime < 1) return distance / 2 *  -Mathf.Pow(2, -10 * (elapsedTime - 1)) + start;
